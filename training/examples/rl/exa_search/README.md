...
- **Reward** (`reward.py`): an **LLM judge** grades the final answer against the
  ...
  formatting). Isolated judge failures fall back to token-F1; persistent ones
  abort the run rather than silently mis-grading it.
...
| `reward.py` | LLM-as-judge answer grading (Fireworks-hosted by default), with a token-F1 offline fallback. |
...
2. Put your keys in `training/.env` (loaded automatically by `train.py`):
   ...
   `JUDGE_MODEL` / `JUDGE_BASE_URL` / `JUDGE_API_KEY`, or pass
   `--no-judge` to grade offline with token-F1.
...
handled too. The judge should be a **non-thinking instruct model** (default:
`qwen3-30b-a3b-instruct-2507`) so its one-word verdict is not swallowed by a
